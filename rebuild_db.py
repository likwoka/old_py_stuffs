...
# Fixed a bug in --rename function, so that renaming unicode characters would
# actually work. 2011-01-20, Alex Li

__title__="KeyJ's iPod shuffle Database Builder"
__version__="1.0-rc1 + Alex's Unicode Patch"
...
def write_to_db(filename):
  ...
  if not entry:
    ...
    ascii_fn = filename.encode("ascii", "ignore")
    entry=header.tostring()+ \
      "".join([c+"\0" for c in ascii_fn[:261]])+ \
      "\0"*(525-2*len(ascii_fn))
...
Please make sure that:
 ...
 (*) you are actually using an iPod shuffle, and not some other iPod model :)""")
  ...
  try:
    if dirs:
      for dir in dirs:
        browse(u"./"+dir,Options['interactive'])
    else:
      browse(u".",Options['interactive'])
